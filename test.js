...
const BNBSwapUSDTorBUSD = BigInt(330);
...
//usdt/busd-swap-wbnb交易排除数组**********需自行设置**********
const EXCLUDED_ADDRESSES = [
  "0x1B96B92314C44b159149f7E0303511fB2Fc4774f",//PancakeLPs
  "0xc590175E458b83680867AFD273527Ff58f74c02b",//
  "0x16b9a82891338f9bA80E2D6970FddA79D1eb0daE",//PancakeLPs
  //"0x58F876857a02D6762E0101bb5C46A8c1ED44Dc16",//PancakeLPs
  "0x05faf555522Fa3F93959F86B41A3808666093210",//PancakeLPs
  "0x348e414212e99fAb834DF43155947a2D6ebA70f9",//PancakeLPs
  "0x61EB789d75A95CAa3fF50ed7E47b96c132fEc082",//PancakeLPs
  "0x7EFaEf62fDdCCa950418312c6C91Aef321375A00",//PancakeLPs
  "0x531FEbfeb9a61D948c384ACFBe6dCc51057AEa7e",//PancakeLPs
  "0x24909Bda809968119E3BeC91cC21C1a2E00ee139",//PancakeLPs
  "0x0eD7e52944161450477ee417DE9Cd3a859b14fD0",//PancakeLPs
  "0x3F803EC2b816Ea7F06EC76aA2B6f2532F9892d62",//PancakeLPs

];
//买入匹配数组
const BUY_METHOD_IDS = [
  "0x5c11d795", // 示例方法ID
  "0x8803dbee",
  "0x5ae401dc",
  "0x791ac947",
];
//transfer swap交易排除数组**********需自行设置**********
const EXCLUDED_METHOD_IDS = [
  "0xa9059cbb", // 示例方法ID
  "0x84477b39",
  "0x4e0b6f07",
];
//买入交易排除数组**********需自行设置**********
const BUY_EXCLUDED_METHOD_IDS = [
  "0xfb3bdb41", // 示例方法ID
  "0xb6f9de95",
];
//卖出交易排除数组**********需自行设置**********
const SELL_EXCLUDED_METHOD_IDS = [
  "0x791ac947", // 示例方法ID
  //"0xb6f9de95",
];
...
async function processTransactions(address, transactions, days, outputFile) {
  ...
  for (const tx of transactions) {
    ...
    //排除0条、1条、2条且swap的transfer事件，只判断2条且不是swap或者超过2条的transfer事件
    const isSwapTransaction = (transferEvents.length === 2 &&
      transferEvents.every(event => {
        const address = event.address.toLowerCase();
        return (
          address === WBNB_CONTRACT_ADDRESS.toLowerCase() ||
          address === USDT_CONTRACT_ADDRESS.toLowerCase() ||
          address === BUSD_CONTRACT_ADDRESS.toLowerCase() ||
          address === BTCB_CONTRACT_ADDRESS.toLowerCase()
        );
      })) || (transferEvents.length === 0 || transferEvents.length === 1)
    ...
    if (transferEvents.length === 0) {
      ...
      console.log("    此交易哈希有两条Transfer事件且是swap交易，跳过判断买入卖出");
    } else if (transferEvents.length === 2) {
      console.log("    此条交易哈希有两条Transfer事件，但不是swap交易，继续买入卖出判断");
    } else {
    ...
    if (shouldHandleBuySellTransaction) {
      for (let i = 0; i < transferEvents.length; i++) {
        ...
        if (i === 0) {
          if ((event.address.toLowerCase() === WBNB_CONTRACT_ADDRESS.toLowerCase() && (from === PANCAKESWAP_ADDRESS || from === MULTICALL_ADDRESS) && !EXCLUDED_ADDRESSES.map(addr => addr.toLowerCase()).includes(to.toLowerCase())) || (event.address.toLowerCase() === WBNB_CONTRACT_ADDRESS.toLowerCase() && (from === PANCAKESWAP_ADDRESS || from === MULTICALL_ADDRESS) && BUY_METHOD_IDS.includes(methodId))) {
            ...
            continue;
          } else if (((event.address.toLowerCase() === USDT_CONTRACT_ADDRESS.toLowerCase() || event.address.toLowerCase() === BUSD_CONTRACT_ADDRESS.toLowerCase()) && from === txDetails.from && !EXCLUDED_ADDRESSES.map(addr => addr.toLowerCase()).includes(to.toLowerCase()) && !EXCLUDED_METHOD_IDS.includes(methodId) && !SELL_EXCLUDED_METHOD_IDS.includes(methodId)) || ((event.address.toLowerCase() === USDT_CONTRACT_ADDRESS.toLowerCase() || event.address.toLowerCase() === BUSD_CONTRACT_ADDRESS.toLowerCase()) && from === txDetails.from && !EXCLUDED_METHOD_IDS.includes(methodId) && !SELL_EXCLUDED_METHOD_IDS.includes(methodId) && BUY_METHOD_IDS.includes(methodId))) {
        ...
        } else if (i === transferEvents.length - 1) {
          if ((event.address.toLowerCase() === WBNB_CONTRACT_ADDRESS.toLowerCase() && (to === PANCAKESWAP_ADDRESS || to === MULTICALL_ADDRESS) && !EXCLUDED_ADDRESSES.map(addr => addr.toLowerCase()).includes(from.toLowerCase())) || (event.address.toLowerCase() === WBNB_CONTRACT_ADDRESS.toLowerCase() && (to === PANCAKESWAP_ADDRESS || to === MULTICALL_ADDRESS) && BUY_METHOD_IDS.includes(methodId))) {
            ...
            matchingConditionCount++;
            continue;
          } else if (((event.address.toLowerCase() === USDT_CONTRACT_ADDRESS.toLowerCase() || event.address.toLowerCase() === BUSD_CONTRACT_ADDRESS.toLowerCase()) && to === txDetails.from && !EXCLUDED_ADDRESSES.map(addr => addr.toLowerCase()).includes(from.toLowerCase()) && !BUY_EXCLUDED_METHOD_IDS.includes(methodId)) || ((event.address.toLowerCase() === USDT_CONTRACT_ADDRESS.toLowerCase() || event.address.toLowerCase() === BUSD_CONTRACT_ADDRESS.toLowerCase()) && to === txDetails.from && !BUY_EXCLUDED_METHOD_IDS.includes(methodId) && BUY_METHOD_IDS.includes(methodId))) {
