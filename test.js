...
const BNBSwapUSDTorBUSD = BigInt(338);
...
////******************************************************************************************************************************************************
//!EXCLUDED_ADDRESSES.map(addr => addr.toLowerCase()).includes(from.toLowerCase())
...
async function processTransactions(address, transactions, days, outputFile) {
  ...
  for (const tx of transactions) {
    ...
    if (shouldHandleBuySellTransaction) {
      const buyIndices = [0, 1]; // 这里可以继续添加买入操作的索引，例如 [0, 1, 3] 等
      const sellIndices = [transferEvents.length - 1, transferEvents.length - 2]; // 这里可以继续添加卖出操作的索引
      for (let i = 0; i < transferEvents.length; i++) {
        ...
        if (buyIndices.includes(i)) {
          if (event.address.toLowerCase() === WBNB_CONTRACT_ADDRESS.toLowerCase() && (from === PANCAKESWAP_ADDRESS || from === MULTICALL_ADDRESS || from === txDetails.to)) {
            ...
            continue;
          } else if ((event.address.toLowerCase() === USDT_CONTRACT_ADDRESS.toLowerCase() || event.address.toLowerCase() === BUSD_CONTRACT_ADDRESS.toLowerCase()) && from === txDetails.from) {
        ...
        } else if (sellIndices.includes(i)) {
