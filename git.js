...
async function processTransactions(address, transactions, days, outputFile) {
  ...
  for (const tx of transactions) {
    ...
    //方便查找同一transfer事件多次匹配的情况,排除0条、1条、大于等于2条且swap的transfer事件，只判断2条且不是swap或者超过2条的transfer事件
    ...
    if (transferEvents.length === 0) {
      console.log("    此条交易哈希没有Transfer事件，或为上链失败、授权、领取空头、充值等操作");
    } else if (transferEvents.length === 1) {
      console.log("    此条交易哈希只有一条Transfer事件，或为地址间转账交易");
    } else if (isSwapTransaction) {
      console.log("    此条交易哈希有大于等于2条Transfer事件且是swap交易，跳过判断买入卖出");
    ...
    if (shouldHandleBuySellTransaction) {
      ...
      for (let i = 0; i < transferEvents.length; i++) {
        ...
        if (buyIndices.includes(i) && i < maxRowsPerJudgment) {
          if (event.address.toLowerCase() === WBNB_CONTRACT_ADDRESS.toLowerCase() && (from === PANCAKESWAP_ADDRESS || from === MULTICALL_ADDRESS || from === txDetails.to || from === txDetails.from)) {
          ...
          if (event.address.toLowerCase() === WBNB_CONTRACT_ADDRESS.toLowerCase() && (to === PANCAKESWAP_ADDRESS || to === MULTICALL_ADDRESS || to === txDetails.to || to === txDetails.from)) {
      ...
      if (!foundMatchingCondition) {
        ...
        console.log("    此transfer事件有多条买入卖出判断");
      }
    }
